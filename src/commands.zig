...
pub fn command_handler(stdout: anytype, user_input: []u8) !void {
    ...
    if (met_command_res == MetaCommandResult.META_COMMAND_UNRECOGNIZED_COMMAND) {
        try stdout.print("Unknown Meta Command: {s}\n", .{user_input});
    }
...
fn prepare_statement(user_input: []u8, statement: *Statement) PrepareResult {
    if (std.mem.eql(u8, user_input, "insert")) {
        statement.type = StatementType.STATEMENT_INSERT;
        return PrepareResult.PREPARE_SUCCESS;
    }

    if (std.mem.eql(u8, user_input, "select")) {
